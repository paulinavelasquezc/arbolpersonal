$(document).ready(function () {
    ...
    $('#btnAdd').click(function () {
        ...
        try {
            ...
            if ($("#ProcessID option:selected").val() === "0") {
                continuar = false;
                $('#ProcessID').siblings('span.error').css('visibility', 'visible');
            }
            else {
                $('#ProcessID').siblings('span.error').css('visibility', 'hidden');
            }

            if (!($('#textCategoria').val().trim() !== '') || ($('#textCategoria').val().trim().length <= 3)) {
                continuar = false;
                $('#textCategoria').siblings('span.error').css('visibility', 'visible');
            }
            else {
                $('#textCategoria').siblings('span.error').css('visibility', 'hidden');
            }
            ...
            if (continuar) {
                ...
                Process.append("Name", $('#textCategoria').val());
                Process.append("ProcessID", $("#ProcessID option:selected").val());

                $.ajax({
                    url: '/CategoryContainer/Save',
                    type: "POST",
                    data: Process,
                    processData: false,
                    contentType: false,
                    ...
                    success: function (data) {
                        if (data.status) {
                            window.location.href = '/CategoryContainer/Index';
                        }
                        else {
