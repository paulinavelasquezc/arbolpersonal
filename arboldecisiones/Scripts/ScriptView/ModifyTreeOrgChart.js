$(document).ready(function () {
    ...
    $('#btnDecisions').click(function () {
        ...
        if (continuar) {
            ...
            $.ajax({
                url: '/ModifyTree/SaveTreeDecision',
                type: "POST",
    ...
    $('#btnUpdateDefect').click(function () {
        ...
        if (continuar) {
            ...
            TreeConfiData.append("TreeConfigurationID", parseInt(sessionStorage.getItem('treeConfigID')));

            $.ajax({
                url: '/ModifyTree/UpdateTreeConfiguration',
                type: "POST",
                data: TreeConfiData,
                processData: false,
                contentType: false,
                ...
                success: function (data) {
                    if (data.status) {
                        ...
                        var id = parseInt(sessionStorage.getItem('treeConfigID'));
    ...
    $('#btnUpdateDecisions').click(function () {
        ...
        if (continuar) {
            ...
            $.ajax({
                url: '/ModifyTree/UpdateTreeDecision',
                type: "POST",
                data: TreeDecisCauseData,
                processData: false,
                contentType: false,
    ...
    $('#btnRemoveDecisions').click(function () {
        var _treeConfigurationID = parseInt(sessionStorage.getItem('treeConfigID'));
        var _treeDecisionID = parseInt(sessionStorage.getItem('TreeDecisionID'));
        $.ajax({
            url: '/ModifyTree/DeleteTreeDecision',
            type: "POST",
                ...
                if (data.status) {
                    ...
                    if (data.remove === 1) {
                        ...
                        BootstrapDialog.confirm({
                            title: 'Eliminar Registros de reporte',
                            message: '¿Esta seguro que desea eliminar, eliminara registros de reportes.?',
                            ...
                            callback: function (result) {
                                if (result) {
                                    ...
                                    //var _treeDecisionID = parseInt(sessionStorage.getItem('TreeDecisionID'));
                                    $.ajax({
                                        url: '/ModifyTree/DeleteDecisionReportRecords',
                                        type: "POST",
                                        data: '{_treeDecisionID: "' + _treeDecisionID + '", _treeConfigurationID: "' + _treeConfigurationID + '" }',
                                        contentType: "application/json; charset=utf-8",
                                        dataType: "json",
                                        success: function (data) {
                                            if (data.status) {
                                                ...
                                                alert('No se puede eliminar ya esta relacionada con reportes.');
                                            }
                                        }, error: function (xhr, ajaxOptions, thrownError) {
                                            alert(xhr.status);
                                            alert(thrownError);
    ...
    $('#removeTree').click(function () {
        BootstrapDialog.confirm({
            title: 'Eliminar árbol decisiones',
            message: '¿Esta seguro que desea eliminar el defecto?',
            type: BootstrapDialog.TYPE_WARNING, // <-- Default value is BootstrapDialog.TYPE_PRIMARY
            closable: true, // <-- Default value is false
            draggable: true, // <-- Default value is false
            btnCancelLabel: 'No', // <-- Default value is 'Cancel',
            ...
            callback: function (result) {
                if (result) {
                    var _id = parseInt(sessionStorage.getItem('treeConfigID'));
                    $.ajax({
                        url: '/ModifyTree/DeleteDecisionTree',
                        type: "POST",
    ...
    function JoinDecisions() {
        var _id = parseInt(sessionStorage.getItem('treeConfigID'));
        $.ajax({
            url: '/ModifyTree/QueryDecisionsXDefect',
            type: "POST",
            data: '{_treeConfigurationID: "' + _id + '" }',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
    ...
    $('#cbxDecisions').change(function () {
        ...
        if (_id !== "0") {
            $.ajax({
                url: '/ModifyTree/QueryTreeDecision',
                type: "POST",
    ...
    $('#btnGuardarJoin').click(function () {
        ...
        if (_id !== 0) {
            ...
            $.ajax({
                url: '/ModifyTree/UpdateJoinDecision',
                type: "POST",
...
function childrenNode(data, node, callback) {

    var Matrixs = Array();
    for (var i = 0; i < data.length; i++) {

        if (data[i].FatherID === node.id) {

            var nodes = new Object();
            nodes.id = data[i].TreeDecisionID;
            nodes.title = data[i].Name;
            nodes.name = data[i].Type;

            //data.splice(i, 1);

            childrenNode(data, nodes, function (value) {
                Matrixs.push(value);
            });

        } else {
            var info = "Eliminar posicion consultar";
        }
    }
    ...
    if (Matrixs.length === 0) {
        ...
        node.children = Matrixs;
        callback(node);
    }
}

function loadTreeDecisions(data) {

    var Matrix = [];
    var array = new Array();

    for (var i = 0; i < data.TreeDecis.length; i++) {

        var node = new Object();
        node.id = data.TreeDecis[i].TreeDecisionID;
        node.title = data.TreeDecis[i].Name;
        node.name = data.TreeDecis[i].Type;


        if (data.TreeDecis[i].FatherID === 0) {
            childrenNode(data.TreeDecis, node, function (value) {
                array.push(value);
            });
        }
    ...
    defecto.id = data.TreeConfi.TreeConfigurationID;
    defecto.title = data.TreeConfi.Name;
    defecto.name = data.TreeConfi.Defecto;
    defecto.children = array;

    sessionStorage.setItem('treeConfigID', data.TreeConfi.TreeConfigurationID);
    ...
    for (var r = 0; r < data.TreeDecis.length; r++) {

        if (data.TreeDecis[r].Type === "UNIÓN") {
            $('#' + data.TreeDecis[r].TreeDecisionID + ' .title').css("background-color", data.TreeDecis[r].ColourHex);
            $('#' + data.TreeDecis[r].TreeDecisionID + ' .content').css("border-color", data.TreeDecis[r].ColourHex);
        }
        if (data.TreeDecis[r].JoinFatherID !== 0) {
            $('#' + data.TreeDecis[r].TreeDecisionID + ' .title').css("background-color", data.TreeDecis[r].ColourHex);
            $('#' + data.TreeDecis[r].TreeDecisionID + ' .content').css("border-color", data.TreeDecis[r].ColourHex);
        }
    }
...
function LoadInfoModalDefect(_treeConfigID) {
    ...
    $.ajax({
        url: '/ModifyTree/QueryTreeConfiguration',
        type: "POST",
        data: '{_treeConfigurationID: "' + _id + '" }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            if (data.status) {
                ...
                $('#LocationID').val(data.treConfig.LocationID);
