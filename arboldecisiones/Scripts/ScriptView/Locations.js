$(document).ready(function () {
    ...
    $('#btnAdd').click(function () {
        ...
        try {
            ...
            if ($("#CategoryContainerID option:selected").val() === "0") {
                continuar = false;
                $('#CategoryContainerID').siblings('span.error').css('visibility', 'visible');
            }
            else {
                $('#CategoryContainerID').siblings('span.error').css('visibility', 'hidden');
            }

            if (!($('#textlocation').val().trim() !== '') || ($('#textlocation').val().trim().length <= 3)) {
                continuar = false;
                $('#textlocation').siblings('span.error').css('visibility', 'visible');
            }
            else {
                $('#textlocation').siblings('span.error').css('visibility', 'hidden');
            }
            ...
            if (continuar) {
                ...
                Process.append("Name", $('#textlocation').val());
                Process.append("CategoryContainerID", $("#CategoryContainerID option:selected").val());
